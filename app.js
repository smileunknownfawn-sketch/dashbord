...
function statusLabel(status) {
  return {done:'Виконано', progress:'В роботі', overdue:'Прострочено'}[status];
}
...
function openOrder(id) {
  ...
  if (!order.fileData) {
    ...
    iframe.src = order.fileData;
    iframe.title = `Розпорядження № ${order.number}`;
    content.appendChild(iframe);
  } else if (order.fileType.startsWith('image/')) {
    const img = document.createElement('img');
    img.src = order.fileData;
    img.alt = `Розпорядження № ${order.number}`;
    ...
    try { text.textContent = atob(order.fileData.split(',')[1]); } catch { text.textContent = 'Не вдалося прочитати текстовий файл.'; }
...
function resetForm() {
  $('orderForm').reset(); selectedFile = null; $('fileTitle').textContent = 'Перетягніть файл сюди або оберіть його'; $('fileError').textContent = '';
}
...
function selectFile(file) {
  ...
  if (file.size > max) { $('fileError').textContent = 'Файл завеликий. Для локальної версії обмеження — 12 МБ.'; return; }
  selectedFile = file; $('fileTitle').textContent = file.name; $('fileError').textContent = '';
}

$('orderForm').addEventListener('submit', async e => {
  e.preventDefault();
  if (!selectedFile) { $('fileError').textContent = 'Прикріпіть файл розпорядження.'; return; }
  const reader = new FileReader();
  reader.onload = () => {
    const order = { id:uid(), number:$('orderNumber').value.trim(), deadline:$('deadline').value, description:$('description').value.trim(), fileName:selectedFile.name, fileType:selectedFile.type || 'application/octet-stream', fileData:reader.result, createdAt:new Date().toISOString(), done:false };
    orders.unshift(order); saveOrders(); orderDialog.close(); render(); showToast('Розпорядження додано до реєстру');
  };
