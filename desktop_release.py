...
from core.analytics import MONTHS_UA, month_comparison, year_comparison, available_years
...
class ReleaseDashboard(ModernDashboard):
    """Фінальна оболонка: теми, налаштування, робоча папка та всі основні дії."""
    ...
    def _install_release_controls(self) -> None:
        ...
        ttk.Label(bar,text="КЕРУВАННЯ",style="CardTitle.TLabel").pack(side="left",padx=(14,8),pady=8)
        self.theme_combo=ttk.Combobox(bar,values=list(THEMES),state="readonly",width=22); self.theme_combo.set(self.theme_name); self.theme_combo.pack(side="left",pady=6); self.theme_combo.bind("<<ComboboxSelected>>",lambda _e:self._theme_selected())
        ttk.Button(bar,text="⚙ Налаштування",command=self.open_settings).pack(side="right",padx=5,pady=6)
        ttk.Button(bar,text="Порівняння",command=self.open_comparison).pack(side="right",padx=5,pady=6)
        ttk.Button(bar,text="Календар / статистика",command=self.open_statistics).pack(side="right",padx=5,pady=6)
        ttk.Button(bar,text="Кошик",command=self.trash_window).pack(side="right",padx=5,pady=6)
        ttk.Button(bar,text="Резервна копія",command=self.create_backup).pack(side="right",padx=5,pady=6)
        ttk.Button(bar,text="Діагностика",command=self.diagnostics).pack(side="right",padx=5,pady=6)
        ttk.Button(bar,text="Відкрити папку",command=self.open_workspace).pack(side="right",padx=5,pady=6)
        ttk.Button(bar,text="Змінити папку",command=self.choose_root).pack(side="right",padx=5,pady=6)
    ...
    def open_statistics(self):
        self.tabs.select(self.analytics_tab)
        self.refresh_analytics()

    def open_comparison(self):
        rows=[dict(r) for r in self.db.get_orders()]
        years=available_years(rows)
        win=tk.Toplevel(self); win.title("Порівняння"); win.geometry("640x430"); win.transient(self); win.grab_set()
        frame=ttk.Frame(win,style="Card.TFrame"); frame.pack(fill="both",expand=True,padx=16,pady=16)
        ttk.Label(frame,text="ПОРІВНЯННЯ ПОКАЗНИКІВ",style="Title.TLabel").pack(anchor="w",padx=18,pady=(18,14))
        grid=ttk.Frame(frame,style="Card.TFrame"); grid.pack(fill="x",padx=18)
        ttk.Label(grid,text="Рік 1").grid(row=0,column=0,padx=5,pady=6,sticky="w"); a=ttk.Combobox(grid,values=[str(x) for x in years],state="readonly",width=14); a.set(str(years[1] if len(years)>1 else years[0])); a.grid(row=0,column=1,padx=5,pady=6)
        ttk.Label(grid,text="Рік 2").grid(row=0,column=2,padx=5,pady=6,sticky="w"); b=ttk.Combobox(grid,values=[str(x) for x in years],state="readonly",width=14); b.set(str(years[0])); b.grid(row=0,column=3,padx=5,pady=6)
        result=tk.StringVar(value="Оберіть роки та натисніть «Порахувати»")
        ttk.Label(frame,textvariable=result,style="CardTitle.TLabel",wraplength=520).pack(anchor="w",padx=18,pady=20)
        def calc():
            data=year_comparison(rows,int(a.get()),int(b.get())); result.set(f"{a.get()}: {data['year_a']} отримано\n{b.get()}: {data['year_b']} отримано\n\nРізниця: {data['difference']:+d}")
        ttk.Button(frame,text="Порахувати",style="Accent.TButton",command=calc).pack(anchor="e",padx=18,pady=8)
        ttk.Separator(frame,orient="horizontal").pack(fill="x",padx=18,pady=10)
        ttk.Label(frame,text="Порівняння місяців",style="CardTitle.TLabel").pack(anchor="w",padx=18)
        mgrid=ttk.Frame(frame,style="Card.TFrame"); mgrid.pack(fill="x",padx=18,pady=6)
        ma=ttk.Combobox(mgrid,values=[f"{i+1:02d} — {MONTHS_UA[i]}" for i in range(12)],state="readonly",width=22); ma.set("01 — Січень"); ma.grid(row=0,column=0,padx=4)
        mb=ttk.Combobox(mgrid,values=[f"{i+1:02d} — {MONTHS_UA[i]}" for i in range(12)],state="readonly",width=22); mb.set("01 — Січень"); mb.grid(row=0,column=1,padx=4)
        def calc_month():
            first=int(ma.get()[:2]); second=int(mb.get()[:2]); data=month_comparison(rows,int(a.get()),first,int(b.get()),second); result.set(f"{a.get()} / {MONTHS_UA[first-1]}: {data['first']}\n{b.get()} / {MONTHS_UA[second-1]}: {data['second']}\n\nРізниця: {data['difference']:+d}")
        ttk.Button(mgrid,text="Порівняти місяці",command=calc_month).grid(row=0,column=2,padx=8)
