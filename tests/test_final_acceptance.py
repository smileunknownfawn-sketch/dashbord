...
def test_100_orders_search_and_status(tmp_path):
    ...
    for i in range(100):
        service.create(
            ...
            description=f"Короткий опис розпорядження для перевірки {i}",
            file_name=f"document_{i}.txt",
            file_bytes=f"Документ {i}".encode("utf-8"),
            responsible=f"Відповідальний {i % 5}",
            tags="контроль тест",
        )
...
def test_attachments_response_trash_restore_and_purge(tmp_path):
    ...
    order_id = service.create(
        number="АТТ-1", received=date(2026, 1, 1), deadline=date(2026, 1, 20),
        description="Перевірка додатків", file_name="order.pdf", file_bytes=b"pdf"
    )
    ...
    assert (paths.orders / "АТТ-1" / "Відповіді" / f"Відповідь_{rid}" / "Додатки").exists()
