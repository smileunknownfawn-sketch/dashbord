...
DEFAULTS = {"workspace": str(DEFAULT_WORKSPACE), "add_open": False, "response_order": None, "language": "Українська", "language_popup": False, "viewer_order": None, "viewer_response": None, "delete_order": None, "edit_order": None, "page": "Головна", "period_year": date.today().year, "period_month": "Усі місяці", "just_added": False}
for key, value in DEFAULTS.items():
    if key not in st.session_state: st.session_state[key] = value

WORK = Path(st.session_state.workspace).resolve(); ORDERS = WORK / "Розпорядження"; BACKUPS = WORK / "Резервні копії"
WORK.mkdir(parents=True, exist_ok=True); ORDERS.mkdir(parents=True, exist_ok=True); BACKUPS.mkdir(parents=True, exist_ok=True); DB = WORK / "database.db"

CSS = """
<style>
html,body,[class*=css]{font-family:"Segoe UI",Arial,sans-serif}.stApp{background:radial-gradient(circle at 85% 5%,rgba(96,125,72,.28),transparent 25%),radial-gradient(circle at 10% 90%,rgba(28,73,55,.22),transparent 30%),linear-gradient(135deg,#030806,#0b1711 48%,#050907);color:#edf4ed}.stApp:before{content:"";position:fixed;inset:0;pointer-events:none;opacity:.12;background-image:linear-gradient(rgba(180,205,180,.18) 1px,transparent 1px),linear-gradient(90deg,rgba(180,205,180,.18) 1px,transparent 1px);background-size:52px 52px}.block-container{max-width:1500px;padding:1.2rem 2.7rem 4rem}.hero{padding:1rem 0 1.2rem}.hero h1{font-size:clamp(2rem,4vw,3.7rem);font-weight:850;letter-spacing:-.045em;margin:.15rem 0 .25rem;font-style:normal}.eyebrow{color:#bdcfac;font-weight:850;font-size:.66rem;letter-spacing:.22em}.version{text-align:right;color:#91a092;font-size:.68rem;letter-spacing:.13em;font-weight:850;padding-top:.3rem}.panel,.metric,.order{background:rgba(7,16,11,.91);border:1px solid rgba(214,232,212,.10);border-radius:18px;padding:1rem;box-shadow:0 15px 40px rgba(0,0,0,.18)}.metric{min-height:100px}.metric .num{font-size:2rem;font-weight:900}.metric .label{color:#9dac9e;font-size:.67rem;font-weight:800;letter-spacing:.1em}.order{margin:.7rem 0;border-left:5px solid #c8a85c}.order.overdue{border-left-color:#e36e68}.order.today{border-left-color:#e1c35f}.order.done{border-left-color:#6db987}.desc{color:#c4cec5;line-height:1.55;margin:.45rem 0}.muted{color:#87958b;font-size:.72rem;letter-spacing:.055em}.badge{display:inline-block;padding:.3rem .62rem;border-radius:999px;font-size:.61rem;font-weight:900}.work{color:#d5b663;background:#d5b66318}.late{color:#ef7770;background:#ef777018}.finished{color:#79c393;background:#79c39318}.todaybadge{color:#e5c968;background:#e5c96818}.priority{color:#d7c48a}.stButton>button{border-radius:10px;background:rgba(120,146,95,.12);border:1px solid rgba(169,188,145,.26);color:#e0e9de;font-weight:850;min-height:42px}.stButton>button:hover{border-color:rgba(218,194,111,.75);background:rgba(208,174,98,.13)}.stTextInput input,.stTextArea textarea,.stDateInput input,.stNumberInput input{background:rgba(0,0,0,.38)!important;color:#fff!important}.stSelectbox div[data-baseweb="select"]>div{background:rgba(0,0,0,.38)}.successbox{padding:16px;border:1px solid #84b88e55;border-radius:14px;background:#1b3925;margin:.5rem 0 1rem}.kpi{font-size:.72rem;color:#aeb9b0}.chartbar{height:14px;border-radius:8px;background:linear-gradient(90deg,#6d8e61,#c9aa5f)}.timeline-item{padding:.7rem 0;border-left:2px solid #6d8e61;padding-left:1rem;margin-left:.4rem}
</style>
"""
...
def db():
    ...
    c.execute("CREATE TABLE IF NOT EXISTS orders(id INTEGER PRIMARY KEY AUTOINCREMENT,number TEXT NOT NULL,deadline TEXT NOT NULL,description TEXT NOT NULL,folder TEXT NOT NULL,filename TEXT NOT NULL,path TEXT NOT NULL,mime TEXT NOT NULL,status TEXT DEFAULT 'progress',completion_outgoing TEXT DEFAULT '',priority TEXT DEFAULT 'Звичайний',responsible TEXT DEFAULT '',category TEXT DEFAULT 'Інше',received_date TEXT DEFAULT '',created_at TEXT NOT NULL)")
    ...
    # Safe migration for databases created by v1.x.
    cols = {r[1] for r in c.execute("PRAGMA table_info(orders)").fetchall()}
    for name, definition in {"priority":"TEXT DEFAULT 'Звичайний'","responsible":"TEXT DEFAULT ''","category":"TEXT DEFAULT 'Інше'","received_date":"TEXT DEFAULT ''"}.items():
        if name not in cols: c.execute(f"ALTER TABLE orders ADD COLUMN {name} {definition}")
    c.commit(); return c


def safe_name(value): return "".join(ch if ch.isalnum() or ch in " ._-()[]" else "_" for ch in str(value).strip()) or "Документ"


def save_upload(upload, folder):
    folder.mkdir(parents=True, exist_ok=True); p = folder / safe_name(upload.name); counter = 2
    while p.exists(): p = folder / f"{p.stem}_{counter}{p.suffix}"; counter += 1
...
def add_order(number,deadline,description,upload,received,priority,responsible,category):
    folder=ORDERS/safe_name(number); counter=2
    while folder.exists(): folder=ORDERS/f"{safe_name(number)}_{counter}"; counter+=1
    p=save_upload(upload,folder); c=db(); cur=c.execute("INSERT INTO orders(number,deadline,description,folder,filename,path,mime,status,priority,responsible,category,received_date,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",(number.strip(),deadline.isoformat(),description.strip(),folder.name,p.name,str(p.relative_to(WORK)),upload.type or mimetypes.guess_type(p.name)[0] or "application/octet-stream","progress",priority,responsible.strip(),category,received.isoformat(),datetime.now().isoformat())); oid=cur.lastrowid; c.commit(); c.close(); event(oid,"Створено",f"№ {number.strip()}")
...
def show_file_viewer(row,response=False):
    ...
    mime=row["mime"] or mimetypes.guess_type(p.name)[0] or "application/octet-stream"; data=p.read_bytes(); st.markdown(f"**{html.escape(p.name)}** · `{mime}`")
...
def play_add_sound():
    ...
    b64=base64.b64encode(audio_file.read_bytes()).decode(); st.markdown(f'<audio autoplay><source src="data:audio/mpeg;base64,{b64}" type="audio/mpeg"></audio>',unsafe_allow_html=True)
...
def status_label(state): return {"done":"ВИКОНАНО","overdue":"ПРОСТРОЧЕНО","today":"ТЕРМІН СЬОГОДНІ","progress":"У РОБОТІ"}[state]


def metric(label,value,hint=""): st.markdown(f'<div class="metric"><div class="label">{label}</div><div class="num">{value}</div><div class="kpi">{html.escape(hint)}</div></div>',unsafe_allow_html=True)

# Ensure old databases are migrated before any SELECT uses new columns.
c=db()
...
with r: st.markdown('<div class="version">ВЕРСІЯ 2.0.1</div>',unsafe_allow_html=True)
...
orders=load_orders(); states=[order_state(o) for o in orders]
...
if st.session_state.add_open:
    with st.form("new_order"):
        st.markdown("### НОВЕ РОЗПОРЯДЖЕННЯ"); a,b,c1=st.columns(3)
        ...
        desc=st.text_area("Короткий опис розпорядження",height=110); f=st.file_uploader("📎 Файл розпорядження")
        x,y=st.columns(2); save=x.form_submit_button("ЗБЕРЕГТИ"); cancel=y.form_submit_button("СКАСУВАТИ")
...
if st.session_state.just_added:
    st.markdown('<div class="successbox"><b>✓ РОЗПОРЯДЖЕННЯ ДОДАНО</b><br><span class="muted">Файл збережено у власній папці розпорядження.</span></div>',unsafe_allow_html=True); play_add_sound(); st.session_state.just_added=False

if st.session_state.page=="Головна":
    overdue=sum(s=="overdue" for s in states); today=sum(s=="today" for s in states); done=sum(s=="done" for s in states); progress=sum(s=="progress" for s in states); m=st.columns(5)
    for col,label,val,hint in [(m[0],"УСЬОГО",len(orders),"усі зареєстровані"),(m[1],"У РОБОТІ",progress,"активні"),(m[2],"СЬОГОДНІ",today,"контрольна дата"),(m[3],"ПРОСТРОЧЕНО",overdue,"потребують уваги"),(m[4],"ВИКОНАНО",done,"закриті")]:
        with col: metric(label,val,hint)
    st.markdown("### 🔴 КОНТРОЛЬНА ПАНЕЛЬ"); critical=[(o,s) for o,s in zip(orders,states) if s in {"overdue","today"}]
    if not critical: st.success("Критичних дедлайнів зараз немає.")
    for o,s in critical[:8]: st.markdown(f'<div class="order {s}"><b>№ {html.escape(o["number"])}</b> · <span class="badge {"late" if s=="overdue" else "todaybadge"}">{status_label(s)}</span><br><span class="desc">{html.escape(o["description"])}</span><br><span class="muted">До: {o["deadline"]} · {days_text(o["deadline"])}</span></div>',unsafe_allow_html=True)
    st.markdown("### 📈 ДИНАМІКА ОТРИМАНИХ"); c=db(); rows=c.execute("SELECT substr(received_date,1,7) ym,COUNT(*) n FROM orders GROUP BY ym ORDER BY ym DESC LIMIT 12").fetchall(); c.close(); maxv=max([r["n"] for r in rows],default=1)
    for row in reversed(rows): st.markdown(f'<div style="display:grid;grid-template-columns:90px 1fr 40px;gap:10px;align-items:center;margin:.35rem 0"><span class="muted">{row["ym"]}</span><div class="chartbar" style="width:{max(4,int(row["n"]*100/maxv))}%"></div><b>{row["n"]}</b></div>',unsafe_allow_html=True)

elif st.session_state.page=="Розпорядження":
    st.markdown("### 📋 РОЗПОРЯДЖЕННЯ"); a,b,c1,d=st.columns([2,1,1,1])
    ...
    filtered=[]
    for o in orders:
        s=order_state(o); hay=f'{o["number"]} {o["completion_outgoing"]} {o["description"]} {o["responsible"]} {o["category"]}'.lower()
        if query and query.lower() not in hay: continue
        if filter_status!="Усі" and status_label(s)!=filter_status: continue
    ...
    st.markdown(f'<div class="muted">ЗНАЙДЕНО: {len(filtered)}</div>',unsafe_allow_html=True)
    for o,s in filtered:
        badge="finished" if s=="done" else "late" if s=="overdue" else "todaybadge" if s=="today" else "work"
        st.markdown(f'<div class="order {s}"><div style="display:flex;justify-content:space-between"><div><b style="font-size:1.15rem">№ {html.escape(o["number"])}</b> <span class="badge {badge}">{status_label(s)}</span></div><span class="priority">{html.escape(o["priority"])}</span></div><div class="desc">{html.escape(o["description"])}</div><div class="muted">До: {o["deadline"]} · {days_text(o["deadline"])} · {html.escape(o["category"])} · {html.escape(o["responsible"] or "Відповідальний не вказаний")}</div></div>',unsafe_allow_html=True)
        ...
        if st.session_state.delete_order==o["id"]:
            st.warning(f"Видалити № {o['number']} разом із файлом та всіма відповідями?"); x,y=st.columns(2)
            with x:
        ...
        if st.session_state.edit_order==o["id"]:
            with st.form(f"editform_{o['id']}"):
                a,b,c1=st.columns(3); nn=a.text_input("№",value=o["number"]); dd=b.date_input("Термін",value=date.fromisoformat(o["deadline"])); pp=c1.selectbox("Пріоритет",["Звичайний","Важливий","Терміновий","Критичний"],index=["Звичайний","Важливий","Терміновий","Критичний"].index(o["priority"])); desc=st.text_area("Короткий опис",value=o["description"]); rr=st.text_input("Відповідальний",value=o["responsible"]); cc=c1.selectbox("Категорія",["Організаційне","Особовий склад","Матеріальне","Навчання","Інше"],index=["Організаційне","Особовий склад","Матеріальне","Навчання","Інше"].index(o["category"])); x,y=st.columns(2); ok=x.form_submit_button("ЗБЕРЕГТИ"); no=y.form_submit_button("СКАСУВАТИ")
...
if st.session_state.viewer_order:
    ...
    if row:
        st.markdown("---"); st.markdown(f"## 📄 РОЗПОРЯДЖЕННЯ № {html.escape(row['number'])}"); st.markdown(f'<div class="panel"><b>Короткий опис</b><p class="desc">{html.escape(row["description"])}</p><span class="muted">Термін: {row["deadline"]} · Статус: {status_label(order_state(row))}</span></div>',unsafe_allow_html=True); show_file_viewer(row); responses=get_responses(row["id"]); st.markdown("### ↩ ВІДПОВІДІ")
...
if st.session_state.response_order:
    ...
    if row:
        ...
        with st.form(f"response_{row['id']}"):
            ...
            comment=st.text_area("Короткий зміст відповіді"); x,y=st.columns(2); save=x.form_submit_button("ЗБЕРЕГТИ ВІДПОВІДЬ"); cancel=y.form_submit_button("СКАСУВАТИ")
...
if st.session_state.page=="Аналітика":
    st.markdown("## 📊 АНАЛІТИКА ВИКОНАННЯ"); a,b,c1=st.columns(3)
    ...
    months=["Січень","Лютий","Березень","Квітень","Травень","Червень","Липень","Серпень","Вересень","Жовтень","Листопад","Грудень"]; data=[]; c=db()
    for i,mn in enumerate(months,1):
        total=c.execute("SELECT COUNT(*) FROM orders WHERE substr(received_date,1,4)=? AND substr(received_date,6,2)=?",(str(int(year)),f"{i:02d}")).fetchone()[0]; finished=c.execute("SELECT COUNT(*) FROM orders WHERE status='done' AND substr(received_date,1,4)=? AND substr(received_date,6,2)=?",(str(int(year)),f"{i:02d}")).fetchone()[0]; late=c.execute("SELECT COUNT(*) FROM orders WHERE substr(received_date,1,4)=? AND substr(received_date,6,2)=? AND deadline < date('now') AND status!='done'",(str(int(year)),f"{i:02d}")).fetchone()[0]; data.append((mn,total,finished,late))
    c.close()
    ...
    for mn,total,finished,late in data:
        st.markdown(f"### {mn} {int(year)}"); vals=[("Отримано",total),("Виконано",finished),("Прострочено",late)] if mode.startswith("Отримано") else [("Отримано",total)] if mode=="Тільки отримані" else [("Виконано",finished)]
        for label,val in vals:
            width=max(3,int(val*100/maxv)); st.markdown(f'<div style="display:grid;grid-template-columns:120px 1fr 45px;gap:10px;align-items:center;margin:.35rem 0"><span class="muted">{label}</span><div class="chartbar" style="width:{width}%"></div><b>{val}</b></div>',unsafe_allow_html=True)
    c=db(); yearly=c.execute("SELECT COUNT(*) total,SUM(CASE WHEN status='done' THEN 1 ELSE 0 END) done FROM orders WHERE substr(received_date,1,4)=?",(str(int(year)),)).fetchone(); c.close(); total=int(yearly["total"] or 0); finished=int(yearly["done"] or 0); rate=(finished/total*100) if total else 0
    st.markdown("### 🎯 ПІДСУМОК РОКУ"); a,b,c1=st.columns(3)
...
if st.session_state.page=="Налаштування":
    st.markdown("## ⚙️ НАЛАШТУВАННЯ"); st.markdown('<div class="panel"><b>📁 Локальне сховище</b><br><span class="muted">Поточна папка даних</span></div>',unsafe_allow_html=True); st.code(str(WORK),language="text")
    if st.button("💾 СТВОРИТИ РЕЗЕРВНУ КОПІЮ"): st.success(f"Резервну копію створено: {make_backup().name}")
    st.markdown("### 🔐 ПРИНЦИПИ ЛОКАЛЬНОЇ РОБОТИ"); st.info("Документи, відповіді та база зберігаються у локальній папці. У застосунку немає API, зовнішніх шрифтів або зовнішніх сервісів. Для .exe буде окремо зафіксовано локальний режим та мережеві обмеження.")
    st.markdown("### 📦 СТРУКТУРА ДАНИХ"); st.code("Дані/\n├── database.db\n├── Розпорядження/\n│   └── <номер>/\n│       ├── основний файл\n│       └── Відповіді/\n└── Резервні копії/",language="text")
