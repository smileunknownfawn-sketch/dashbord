...
for key, value in {"workspace": str(DEFAULT_WORKSPACE), "add_open": False, "response_order": None, "language": "Українська", "language_popup": False, "just_added": False, "viewer_order": None, "viewer_response": None, "delete_order": None}.items():
...
CSS = '''<style>:root{--text:#edf2ec;--muted:#8c9a90;--gold:#d0ae62;--red:#e36e68;--green:#79b88a}html,body,[class*=css]{font-family:Inter,"Segoe UI",Arial,sans-serif}.stApp{background:radial-gradient(circle at 80% 10%,#263c2b 0,transparent 28%),linear-gradient(135deg,#050a06,#142118 50%,#050806);color:var(--text)}.block-container{max-width:1480px;padding:1.5rem 3rem 3rem}.hero h1{font-family:"Segoe UI",Inter,Arial,sans-serif;font-style:normal;font-weight:800;font-size:clamp(2.1rem,4vw,3.7rem);letter-spacing:-.045em;margin:.2rem 0 .3rem}.eyebrow{color:#b8cba9;font-weight:800;font-size:.66rem;letter-spacing:.20em}.version{text-align:right;color:#8e9a91;font-size:.68rem;letter-spacing:.12em;font-weight:800}.panel,.order,.metric{background:rgba(8,18,12,.93);border:1px solid #dce9d91c;border-radius:16px;padding:1rem}.metric{min-height:90px}.order{margin:.75rem 0 .25rem;border-left:5px solid var(--gold);box-shadow:0 10px 30px #0003}.order.overdue{border-left-color:var(--red)}.order.done{border-left-color:var(--green)}.desc{color:#c3cdc6;margin:.55rem 0;line-height:1.55}.muted{color:var(--muted);font-size:.70rem;letter-spacing:.06em}.badge{padding:.28rem .58rem;border-radius:999px;font-size:.62rem;font-weight:850}.work{color:var(--gold);background:#d0ae6218}.late{color:var(--red);background:#e36e6818}.finished{color:var(--green);background:#79b88a18}.stButton>button{border-radius:9px;background:#78925f20;border:1px solid #a9bc9140;color:#dce7d8;font-weight:800;min-height:42px}.stButton>button:hover{border-color:#d0ae62aa;background:#d0ae6215}.successbox{padding:18px;border:1px solid #9ab58a55;border-radius:14px;background:#263d2a;margin:.5rem 0 1rem}.dangerbox{padding:14px;border:1px solid #e36e6855;border-radius:14px;background:#3a1715;margin:.5rem 0}.stTextInput input,.stTextArea textarea,.stDateInput input{background:#0005!important;color:#fff!important}</style>'''
...
def add_order(number,deadline,description,upload):
    ...
    p=save_upload(upload,folder); c=db(); c.execute('INSERT INTO orders(number,deadline,description,folder,filename,path,mime,status,created_at) VALUES(?,?,?,?,?,?,?,?,?)',(number.strip(),deadline.isoformat(),description.strip(),folder.name,p.name,str(p.relative_to(WORK)),upload.type or mimetypes.guess_type(p.name)[0] or 'application/octet-stream','progress',datetime.now().isoformat())); c.commit(); c.close()
...
def delete_order(order_id):
    ...
    except ValueError: c.close(); raise RuntimeError('Небезпечний шлях папки розпорядження')
...
def play_add_sound():
    ...
    b64=base64.b64encode(audio_file.read_bytes()).decode(); st.markdown(f'<audio autoplay controls style="display:none"><source src="data:audio/mpeg;base64,{b64}" type="audio/mpeg"></audio>',unsafe_allow_html=True)
...
with right: st.markdown('<div class="version">ВЕРСІЯ 1.2</div>',unsafe_allow_html=True)
if st.session_state.language_popup and st.session_state.language=='Російська':
    ...
    else: st.warning('Файл language-russian.jpg не знайдено у папці програми.'); st.session_state.language_popup=False
...
c=db(); rows=c.execute('SELECT * FROM orders ORDER BY deadline').fetchall(); c.close(); counts={'progress':0,'today':0,'overdue':0,'done':0}
for r in rows: counts[order_state(r)]+=1
for col,label,val in zip(st.columns(4),['УСЬОГО','У РОБОТІ','ПРОСТРОЧЕНО','ВИКОНАНО'],[len(rows),counts['progress']+counts['today'],counts['overdue'],counts['done']]):
...
st.write(''); q=st.text_input('🔎 Пошук за №, описом, вихідним номером або файлом',placeholder='Введіть текст...'); filt=st.radio('ФІЛЬТР',['Усі','У роботі','Прострочені','Виконані'],horizontal=True)

for r in rows:
    s=order_state(r); responses=get_responses(r['id']); hay=f"{r['number']} {r['description']} {r['filename']} {r['completion_outgoing']} "+' '.join((x['outgoing']+' '+x['filename']+' '+x['comment']) for x in responses)
    if q.lower() not in hay.lower() or (filt=='У роботі' and s not in ('progress','today')) or (filt=='Прострочені' and s!='overdue') or (filt=='Виконані' and s!='done'): continue
    cls='overdue' if s=='overdue' else ('done' if s=='done' else ''); badge='late' if s=='overdue' else ('finished' if s=='done' else 'work'); label={'overdue':'ПРОСТРОЧЕНО','done':'ВИКОНАНО','today':'ТЕРМІН СЬОГОДНІ','progress':'У РОБОТІ'}[s]
    ...
    a,b,c=st.columns(3)
    with a:
        if st.button('📄 ПЕРЕГЛЯНУТИ РОЗПОРЯДЖЕННЯ',key=f'open{r["id"]}'): st.session_state.viewer_order=r['id']; st.rerun()
    with b:
    ...
    if st.session_state.delete_order==r['id']:
        st.markdown(f'<div class="dangerbox"><b>УВАГА!</b><br>Видалити розпорядження № {html.escape(r["number"])} разом з файлом та всіма відповідями?<br><span class="muted">Цю дію не можна скасувати.</span></div>',unsafe_allow_html=True)
        yes,no=st.columns(2)
        with yes:
            if st.button('ТАК, ВИДАЛИТИ',key=f'confirmdel{r["id"]}'):
                delete_order(r['id']); st.session_state.delete_order=None; st.session_state.viewer_order=None; st.session_state.response_order=None; st.rerun()
        with no:
            if st.button('НІ, СКАСУВАТИ',key=f'canceldel{r["id"]}'):
                st.session_state.delete_order=None; st.rerun()
    if st.session_state.viewer_order==r['id']:
        with st.expander('📄 ПЕРЕГЛЯД ДОКУМЕНТА',expanded=True):
            show_file_viewer(r)
            if st.button('✕ ЗАКРИТИ ПЕРЕГЛЯД',key=f'closev{r["id"]}'): st.session_state.viewer_order=None; st.rerun()
    if responses:
        with st.expander(f'📎 ВІДПОВІДІ ({len(responses)})',expanded=False):
            for rr in responses:
                st.markdown(f"**{rr['response_date']}** · Вих. № **{html.escape(rr['outgoing']) if rr['outgoing'] else 'не вказано'}** — {html.escape(rr['comment'])}")
                if st.button('👁 ПЕРЕГЛЯНУТИ ВІДПОВІДЬ',key=f'vr{rr["id"]}'): st.session_state.viewer_response=rr['id']; st.session_state.viewer_order=None; st.rerun()
                if st.session_state.viewer_response==rr['id']: show_file_viewer(rr,response=True)
    if st.session_state.response_order==r['id']:
        with st.form(f'form{r["id"]}'):
            d=st.date_input('Дата відповіді',value=date.today(),key=f'date{r["id"]}'); out=st.text_input('Вихідний номер відповіді',key=f'out{r["id"]}'); comment=st.text_area('Короткий зміст відповіді',key=f'com{r["id"]}'); rf=st.file_uploader('Файл відповіді / архів',key=f'file{r["id"]}'); final=st.checkbox('Позначити розпорядження виконаним',value=False,key=f'fin{r["id"]}'); ok=st.form_submit_button('ЗБЕРЕГТИ ВІДПОВІДЬ')
            if ok:
