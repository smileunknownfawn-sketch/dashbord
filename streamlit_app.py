...
if lang=='Російська':
 st.warning('Ти що, москаль? 😄')
 st.components.v1.html('''<script>setTimeout(()=>{const a=document.querySelectorAll('audio');},3500)</script>''',height=1)
 ...
 with st.form('new'):
  ...
  if save:
   if not n or not desc or not f:st.error('Заповніть №, короткий опис та додайте файл.')
    ...
    add_order(n,d,desc,f);st.session_state.add_open=False;st.session_state.just_added=True;st.rerun()
if st.session_state.pop('just_added',False):
 ...
 audio(APP/'sounds'/'opiat-rabota.mp3')
...
for r in rows:
 ...
 with c:
  if s!='done' and st.button('✓ ПОЗНАЧИТИ ВИКОНАНИМ',key=f'done{r["id"]}'):
   st.session_state.response_order=r['id'];st.session_state.final_response=True;st.rerun()
