...
for key, value in {"workspace": str(DEFAULT_WORKSPACE), "add_open": False, "response_order": None, "language": "Українська", "language_popup": False, "just_added": False, "settings_open": False}.items():
    if key not in st.session_state: st.session_state[key] = value
WORK = Path(st.session_state.workspace); ORDERS = WORK / "Розпорядження"; BACKUPS = WORK / "Резервні копії"
...
def add_order(number,deadline,description,upload):
    ...
    p=save_upload(upload,folder); c=db(); c.execute('INSERT INTO orders(number,deadline,description,folder,filename,path,mime,status,created_at) VALUES(?,?,?,?,?,?,?,?,?)',(number,deadline.isoformat(),description,folder.name,p.name,str(p.relative_to(WORK)),upload.type or mimetypes.guess_type(p.name)[0] or 'application/octet-stream','progress',datetime.now().isoformat())); c.commit(); c.close()
...
with right: st.markdown('<div class="version">ВЕРСІЯ 1.1</div>',unsafe_allow_html=True)
...
for r in rows:
    ...
    a,b,c=st.columns(3)
    with a:
        if st.button('📄 ПЕРЕГЛЯНУТИ РОЗПОРЯДЖЕННЯ',key=f'open{r["id"]}'):st.session_state['viewer_order']=r['id'];st.session_state['viewer_response']=None;st.rerun()
    with b:
    ...
    if st.session_state.get('viewer_order')==r['id']:
    ...
    if responses:
        with st.expander(f'📎 ВІДПОВІДІ ({len(responses)})',expanded=False):
            for rr in responses:
                ...
                if st.session_state.get('viewer_response')==rr['id']:show_file_viewer(rr,response=True)
...
st.markdown('<div class="muted" style="text-align:center;margin-top:3rem">СИСТЕМА КОНТРОЛЮ ВИКОНАННЯ • ЛОКАЛЬНИЙ РЕЖИМ • ВЕРСІЯ 1.1</div>',unsafe_allow_html=True)
