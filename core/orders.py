...
class OrderService:
    ...
    def move_to_trash(self, order: Any) -> Path:
        ...
        counter = 2
        while target.exists():
            target = self.storage.paths.trash / f"{datetime.now():%Y%m%d_%H%M%S}_{self.storage.clean_name(order['number'])}_{counter}"
            counter += 1
        source.rename(target)
        self.db.move_to_trash(order["id"], order["number"], order["folder"], str(target.relative_to(self.storage.paths.root)))
        return target
