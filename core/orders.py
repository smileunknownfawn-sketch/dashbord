...
class OrderService:
    def __init__(self,db:Database,storage:LocalStorage): self.db=db; self.storage=storage
    @staticmethod
    def status(row:Any,today:date|None=None)->str:
        if str(row["status"]).lower() in DONE:return "done"
        anchor=today or date.today()
        try: deadline=date.fromisoformat(str(row["deadline"]))
        except (TypeError,ValueError): return "progress"
        if deadline<anchor:return "overdue"
        if deadline==anchor:return "today"
        return "progress"
    @staticmethod
    def remaining(value:Any,today:date|None=None)->str:
        try:left=(date.fromisoformat(str(value))-(today or date.today())).days
        except (TypeError,ValueError):return "Термін не вказано"
    ...
    def create(self,*,number:str,received:date,deadline:date,description:str,file_name:str,file_bytes:bytes,mime:str="",priority:str="Звичайний",responsible:str="",category:str="Інше",tags:str="")->int:
        ...
        folder_name=self.storage.clean_name(number); folder=self.storage.paths.orders/folder_name; counter=2
        while folder.exists():folder=self.storage.paths.orders/f"{folder_name}_{counter}";counter+=1
        folder.mkdir(parents=True,exist_ok=True); document=self.storage.save_bytes(folder,file_name,file_bytes); mime=mime or mimetypes.guess_type(document.name)[0] or "application/octet-stream"; now=datetime.now().isoformat(timespec="seconds")
        oid=self.db.execute("INSERT INTO orders(number,deadline,description,folder,filename,path,mime,status,priority,responsible,category,received_date,tags,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",(number,deadline.isoformat(),description,folder.name,document.name,str(document.relative_to(self.storage.paths.root)),mime,"progress",priority,responsible.strip(),category,received.isoformat(),tags.strip(),now,now)); self.db.log(oid,"Створено",f"№ {number}"); return oid
    def update(self,order_id:int,*,number:str,deadline:date,description:str,priority:str,responsible:str,category:str,tags:str)->None:
        ...
        self.db.execute("UPDATE orders SET number=?,deadline=?,description=?,priority=?,responsible=?,category=?,tags=?,updated_at=? WHERE id=?",(number.strip(),deadline.isoformat(),description.strip(),priority,responsible.strip(),category,tags.strip(),datetime.now().isoformat(timespec="seconds"),order_id));self.db.log(order_id,"Змінено","Оновлено реквізити розпорядження")
    def add_response(self,order:Any,*,response_date:date,outgoing:str,comment:str,file_name:str,file_bytes:bytes,mime:str="",final:bool=False)->int:
        folder=self.storage.paths.orders/order["folder"]/"Відповіді";document=self.storage.save_bytes(folder,file_name,file_bytes);mime=mime or mimetypes.guess_type(document.name)[0] or "application/octet-stream"
    ...
    def move_to_trash(self,order:Any)->Path:
        ...
        target=self.storage.paths.trash/f"{datetime.now():%Y%m%d_%H%M%S}_{self.storage.clean_name(order['number'])}";counter=2
        while target.exists():target=self.storage.paths.trash/f"{target.name}_{counter}";counter+=1
        source.rename(target);self.db.move_to_trash(order["id"],str(target.relative_to(self.storage.paths.root)));return target
    def restore(self,order_id:int)->Path:
        ...
        if not source.exists():raise FileNotFoundError("Файли розпорядження у кошику не знайдено")
        ...
        source.rename(target);self.db.restore_from_trash(order_id);return target
