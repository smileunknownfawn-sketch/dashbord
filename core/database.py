...
SCHEMA = {
    "orders": """CREATE TABLE IF NOT EXISTS orders(id INTEGER PRIMARY KEY AUTOINCREMENT,number TEXT NOT NULL,deadline TEXT NOT NULL,description TEXT NOT NULL,folder TEXT NOT NULL,filename TEXT NOT NULL,path TEXT NOT NULL,mime TEXT NOT NULL,status TEXT DEFAULT 'progress',completion_outgoing TEXT DEFAULT '',priority TEXT DEFAULT 'Звичайний',responsible TEXT DEFAULT '',category TEXT DEFAULT 'Інше',received_date TEXT DEFAULT '',tags TEXT DEFAULT '',created_at TEXT NOT NULL,updated_at TEXT DEFAULT '',deleted_at TEXT DEFAULT '')""",
...
MIGRATIONS = {"orders":{"completion_outgoing":"TEXT DEFAULT ''","priority":"TEXT DEFAULT 'Звичайний'","responsible":"TEXT DEFAULT ''","category":"TEXT DEFAULT 'Інше'","received_date":"TEXT DEFAULT ''","tags":"TEXT DEFAULT ''","updated_at":"TEXT DEFAULT ''","deleted_at":"TEXT DEFAULT ''"},"responses":{"is_final":"INTEGER DEFAULT 0"}}
...
class Database:
    ...
    def get_orders(self, include_deleted: bool = False) -> list[sqlite3.Row]:
        ...
        return self.fetchall(f"SELECT * FROM orders {where} ORDER BY CASE WHEN status='done' THEN 2 ELSE 0 END, deadline ASC, id DESC")

    def get_order(self, order_id: int, include_deleted: bool = False) -> sqlite3.Row | None:
        where = "" if include_deleted else "AND COALESCE(deleted_at,'')=''"
        return self.fetchone(f"SELECT * FROM orders WHERE id=? {where}", (order_id,))
    ...
    def move_to_trash(self, order_id: int) -> None:
        ...
        with self.connection() as conn:
            conn.execute("UPDATE orders SET deleted_at=?,updated_at=? WHERE id=?", (now,now,order_id))
            conn.execute("INSERT INTO events(order_id,event_type,details,created_at) VALUES(?,?,?,?)", (order_id,"Переміщено до кошика","Розпорядження можна відновити",now))

    def restore_from_trash(self, order_id: int) -> None:
        self.execute("UPDATE orders SET deleted_at='',updated_at=? WHERE id=?", (datetime.now().isoformat(timespec="seconds"),order_id)); self.log(order_id,"Відновлено","Розпорядження повернуто з кошика")
    ...
    def remove_trash_record(self, order_id: int) -> None:
        self.purge_order(order_id)
